...
class source:
	...
	def results(self, info):
		try:
			...
			from modules.kodi_utils import logger
			logger('NXTFlix alldebrid scraper Exception', str(e))
		internal_results(self.scrape_provider, self.sources)
		return self.sources

	def _gather_assigned_content(self):
		pre_assigned_content = gather_assigned_content("'NXTFlix_AD_%'")
	...
	def _scrape_cloud(self):
		try:
			...
			for item in my_cloud_files:
				...
				if test_assigned_content('NXTFlix_AD_%s' % item['id'], self.pre_assigned_content): results_append((item, True))
